...
_NOTE: I know this post is very "me, me, me". But that's the easiest way to write. It'll get better with time..._

## I Need a Better Browser

This weekend I came across a recent status update video from the Ladybird browser project. Because it had been a while since my last attempt, I decided to try to fire up their browser again: `nix run nixpkgs#ladybird`. Compiling... compiling... window pops up... boom! CRASH :(

This is the same problem I've been having ever since I started using NixOS last fall, and I've not dug far enough into the nixpkgs derivation or Ladybird repo or whereever to find out what's the cause. And I wasn't gonna do that this time either.

Now, the Ladybird status update also mentioned my other favorite indie browser Servo (not NetSurf). Servo starts just fine on my machine, but using it for anything at all really is a pain. And they just don't seem to have even nearly the pace of improvement that Ladybird enjoys.
...
Despite all that, I did not want to give up. I really do need a better browser. We all do. If I could only just at least load my own dead simple pure HTML sites when I am developing, then that would be a great step. I don't need the JavaScript monstrosities to run well, just the simple stuff.
...
I am very comfortable with Rust. And not only that, Servo also have [this page](https://starters.servo.org/) describing simple beginner issues. They basically funnel you into that page starting with a big green "Get Involved" button on the default Servo browser new tab page (servo.org).
...
And that was that, more or less. I guess the message is to just try things, you will probably learn something, and it doesn't have to be so difficult.
